 ...
 * reach half of that exactly touches its neighbour.
 *
 * 0.87 of it, not 0.98. Where the stars meet, their black joins into one field
 * and from some angles the ball inverts into white marks on black. Pulling them
 * apart leaves the white cross the real ball has between every pair, and that
 * white is what keeps the pattern legible while the ball turns.
 */
const TOUCH = Math.acos(1 / Math.sqrt(5)) / 2;
const STAR_OUTER = TOUCH * 0.87;
/**
